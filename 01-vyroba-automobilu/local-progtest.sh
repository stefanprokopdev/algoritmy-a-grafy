...
do
    let i=$i+1;
	li=`printf %d $i`
	if [ -r sample/input"$li" ]; then
		./"$appFile" < sample/"input$li" > sample/myoutput.txt;
		diff sample/myoutput.txt sample/"output$li" > /dev/null;
		if [ "$?" = "1" ]; then		
			echo "------------------------------------ $li -------------------------------------";
			cat "sample/input$li";
			echo "-  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -";		
			diff sample/myoutput.txt sample/output"$li";
			let errors+=1;
		fi
		let count=$count+1
	else
